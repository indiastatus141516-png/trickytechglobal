...
const DatePicker = ({ value, onChange, className, placeholder }) => {
  const [showPicker, setShowPicker] = useState(false);
  
  // Convert MM-DD-YYYY to YYYY-MM-DD for native input
  const convertToInputValue = (mmddyyyy) => {
    if (!mmddyyyy) return '';
  ...
  // Convert YYYY-MM-DD to MM-DD-YYYY for display and storage
  const convertFromInputValue = (yyyymmdd) => {
    if (!yyyymmdd) return '';
  ...
  const handleDateChange = (e) => {
    ...
    const mmddyyyy = convertFromInputValue(yyyymmdd);
    onChange(mmddyyyy);
    setShowPicker(false);
  };
...
const ProfileForm = ({ defaultValues = {}, onSubmit = () => {}, isProfileCompleted = false }) => {
  ...
  const handleDateOfBirthChange = (mmddyyyy) => {
    setDateOfBirth(mmddyyyy);
    setValue('dateOfBirth', mmddyyyy);
  };
    ...
    return (
      <form className="profile-form" onSubmit={handleSubmit((data) => {
        ...
        <div className="profile-grid">
          ...
          <div className="profile-field">
            ...
            <div className="phone-row">
              <select className="phone-code-select" {...register('phoneCountryCode')} defaultValue={defaultValues.phoneCountryCode || '+1'}>
                <option value="+1">+1</option>
                <option value="+44">+44</option>
                <option value="+91">+91</option>
                <option value="+61">+61</option>
          ...
          <div className="profile-field">
            ...
            />
            <input type="hidden" {...register('dateOfBirth')} value={dateOfBirth} />
            {errors.dateOfBirth && <div className="field-error">{errors.dateOfBirth.message}</div>}
          </div>
          ...
          {educationLocal.map((edu, idx) => (
            <div key={idx} className="education-card">
              <div className="education-row">
                ...
                <div className="education-col education-col-wide">
                  <label className="field-label">School</label>
                  <input className="field-input" value={edu.school} onChange={e => updateEducationItem(idx, 'school', e.target.value)} />
                </div>
              </div>
              <div className="education-row education-row-spaced">
                <div className="education-col education-col-wide">
                  <label className="field-label">University with City and zip code</label>
                  <input className="field-input" value={edu.universityCityZip} onChange={e => updateEducationItem(idx, 'universityCityZip', e.target.value)} />
                </div>
                <div className="education-col">
                  ...
                  <DatePicker
                    value={edu.startDate}
                    onChange={(value) => updateEducationItem(idx, 'startDate', value)}
                    className="field-input"
                    placeholder="MM-DD-YYYY"
                  />
                </div>
                <div className="education-col">
                  ...
                  <DatePicker
                    value={edu.endDate}
                    onChange={(value) => updateEducationItem(idx, 'endDate', value)}
                    className="field-input"
                    placeholder="MM-DD-YYYY"
                  />
                </div>
              ...
              <div className="education-actions">
                <button type="button" onClick={() => removeEducation(idx)} className="btn-danger">Remove</button>
              </div>
            </div>
          ))}

          <div className="education-add-row">
            <button type="button" onClick={() => addEducation('Bachelor')} className="btn-add btn-add-primary">+ Add Bachelor</button>
            <button type="button" onClick={() => addEducation('Master')} className="btn-add btn-add-primary">+ Add Master</button>
            <button type="button" onClick={() => addEducation('Other')} className="btn-add btn-add-secondary">+ Add Education</button>
          </div>
          {educationErrors.length > 0 && (
            <div style={{ color: 'red', marginTop: 8 }}>
              {educationErrors.map((error, idx) => <div key={idx}>{error}</div>)}
        ...
        <div className="profile-grid">
          ...
          <div className="profile-field">
            <label className="field-label">When did you come to USA? (MM-YY) <span className="required">*</span></label>
            <input className="field-input" {...register('whenCameToUSA')} defaultValue={defaultValues.whenCameToUSA || ''} type="month" />
            {errors.whenCameToUSA && <div className="field-error">{errors.whenCameToUSA.message}</div>}
          </div>
          <div className="profile-field">
            <label className="field-label">Work Authorization (Visa Status) <span className="required">*</span></label>
  ...
  return (
    <div className="profile-container">
      ...
      <div className="profile-hero">
        <h2 className="profile-hero-title">{localValues.firstName} {localValues.lastName}</h2>
        <p className="profile-hero-subtitle">{localValues.email}</p>
        <p className="profile-hero-meta">{localValues.city}, {localValues.state}</p>
      </div>
      {sections.map(section => (
        <div key={section.title} className="profile-section">
          ...
          <div className="profile-section-grid">
            {section.fields.map(fieldName => {
              ...
              if (!field) return null;
              return (
                <div key={field.name} className="profile-field-card">
